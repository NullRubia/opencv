...
#GaussianBlur
# 픽셀 주변 값들을 가우시안 분포에 따라 가중 편균에서 흐리게 만듦
# 커널 크기가 클수록 넓은 범위를 흐리게 함
# 전체적으로 흐림 처리만 하고 싶을 때
dst3 = cv2.GaussianBlur(img, (0,0), 2)

#medianBlur
# 픽셀 주면의 값 중에서 중간값을 선택해서 새로운 픽셀 값으로 바꾸는 필터
# 이미지의 각 픽셀을 기준으로 주변 픽셀을 확인한 후 그 중에서 가장 중간값(순서대로 정렬)을 가져와서 그 픽셀을 새로운 값으로 바꿔 블러 처리함
dst4 = cv2.medianBlur(img, 7)

#양방향 필터(bilateralFilter)
# 엣지(윤곽선) 보전하면서 부드럽게 처리할 수 있는 필터
# 공간 정보+ 픽셀 색상 정보를 함께 고려
# 색상 거리 시그마, 공간거리 시그마(100, 100)
# 노이즈를 줄이고, 윤곽선을 유지하고 싶을 때
dst5 = cv2.bilateralFilter(img, 15, 100, 100)

# Canny 엣지 검출
# 엣지를 찾기 위함 임계값을 자동으로 조정하여 엣지를 찾아주는 알고리즘
